...
<div class="space-y-4">
	...
	{#if activeView === 'chart' && latest?.tdee_harris_benedict}
		<div class="rounded-xl border border-white/[0.06] bg-[#35403B] p-4">
			<h3 class="mb-4 text-sm font-semibold text-[#FDFBF7]/70">TDEE Comparison (Latest Measurement)</h3>
						...
						<div class="flex items-center gap-4">
							<div>
								...
								<div class="mt-0.5 flex items-center gap-3">
									{#if m.bmi != null}
										<span class="text-xs text-[#FDFBF7]/50">BMI: <span class="clinical">{formatBMI(m.bmi)}</span></span>
									{/if}
									{#if m.weight_kg != null}
										<span class="text-xs text-[#FDFBF7]/50">Weight: <span class="clinical">{m.weight_kg.toFixed(2)} kg</span></span>
									{/if}
									{#if m.bmi_category}
					...
					{#if expandedId === m.id}
						<div class="border-t border-white/[0.06] p-4">
							<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
								{#if m.height_cm != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.height_cm.toFixed(1)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">Height cm</p></div>{/if}
								{#if m.weight_kg != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.weight_kg.toFixed(2)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">Weight kg</p></div>{/if}
								{#if m.bmi != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{formatBMI(m.bmi)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">BMI</p></div>{/if}
								{#if m.waist_cm != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.waist_cm.toFixed(1)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">Waist cm</p></div>{/if}
								{#if m.hip_cm != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.hip_cm.toFixed(1)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">Hip cm</p></div>{/if}
								{#if m.bmr_harris_benedict != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.bmr_harris_benedict.toFixed(0)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">BMR (HB)</p></div>{/if}
								{#if m.bmr_mifflin_st_jeor != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.bmr_mifflin_st_jeor.toFixed(0)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">BMR (MSJ)</p></div>{/if}
								{#if m.healthy_weight != null}<div class="rounded-lg bg-[#2C3531]/60 p-3 text-center"><p class="clinical text-base">{m.healthy_weight.toFixed(1)}</p><p class="text-[10px] uppercase tracking-wide text-[#FDFBF7]/40">Healthy kg</p></div>{/if}
							</div>
						</div>
					{/if}
				</div>
