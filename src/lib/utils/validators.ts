...
export function isEmail(value: string, label = 'Email'): string | null {
	if (!value) return null;
	const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
	if (!re.test(value)) return `${label} is not a valid email address`;
	return null;
}
...
export function isPhone(value: string, label = 'Phone'): string | null {
	if (!value) return null;
	const re = /^\+?[\d\s\-().]{7,20}$/;
	if (!re.test(value)) return `${label} is not a valid phone number`;
	return null;
}

export function isSsnLastFour(value: string): string | null {
	if (!value) return null;
	if (!/^\d{4}$/.test(value)) return 'SSN last four must be exactly 4 digits';
	return null;
}
...
export function validatePatientCreate(data: Record<string, unknown>): ValidationResult {
	...
	if (dobErr) {
		...
		const dateErr = isDate(String(data.date_of_birth), 'Date of birth');
	...
	const phoneErr = required(ci?.phone_number, 'Phone number');
	if (phoneErr) {
		errors.phone_number = phoneErr;
	} else {
		const pErr = isPhone(String(ci?.phone_number ?? ''), 'Phone number');
		if (pErr) errors.phone_number = pErr;
	}

	if (ci?.email) {
		const eErr = isEmail(String(ci.email), 'Email');
...
export function validateMeasurementCreate(data: Record<string, unknown>): ValidationResult {
	...
	if (!hasAny) {
		errors.general = 'At least one measurement (height, weight, waist, or hip) is required';
	}
	return { valid: Object.keys(errors).length === 0, errors };
}
